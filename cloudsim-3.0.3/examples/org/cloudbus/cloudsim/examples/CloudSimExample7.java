		...
		try {
			...
			Runnable monitor = new Runnable() {
				...
				public void run() {
					...
					Log.printLine("the pauseSimulation time: "+CloudSim.clock());
					...
					DatacenterBroker newbroker = createBroker("Broker_1");
					int brokerId = newbroker.getId();
					//Create VMs and Cloudlets and send them to broker
					
					vmlist = createVM(brokerId, 5, 100); //creating 5 vms
					//broker.getVmList().addAll(vmlist);
					
					cloudletList = createCloudlet(brokerId, 10, 100); // creating 10 cloudlets
					
					//broker.getCloudletReceivedList().addAll(cloudletList);

					newbroker.submitVmList(vmlist);
					newbroker.submitCloudletList(cloudletList);
