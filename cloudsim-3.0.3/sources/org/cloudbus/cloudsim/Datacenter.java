...
public class Datacenter extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			...
			case CloudSimTags.VM_DATA_DEL_ACK:
				...
				break;
			// new extends: handle the broker requests
	...
	protected void processVmBroker(SimEvent ev) {
		Vm vm = (Vm) ev.getData();
		int data[] = new int[3];
		data[0] = getId();
		data[1] = vm.getId();
		if(VmList.getById(this.getVmList(), vm.getId()) == null)
			data[2] = CloudSimTags.FALSE;
		else
			data[2] = CloudSimTags.TRUE;
		send(vm.getUserId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.VM_BROKER_EVENT, data);
	}
	/**
	 * Process data del.
	 * 
